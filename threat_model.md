...
- **Tenant user-management metadata** — member email addresses, roles, full-access state, account activity, and tenant contact information. This data can support internal reconnaissance and targeted phishing if overexposed.
...
- **Public registrant to provisioned tenant boundary** — self-service registration creates new tenants and quota-bearing admin accounts. Provisioning and defaults must not let arbitrary internet users turn account creation into privileged abuse or platform-wide resource exhaustion.
...
- **Current scan notes:** treat production boot/session-secret handling and the implemented-vs-claimed account-lockout behavior as active auth review anchors on future scans. The codebase currently contains a production session-secret fallback and lockout fields/constants that are not enforced in the login path.
...
Users can change assessments, incidents, supplier records, evidence state, and tenant settings. The server must treat the client as untrusted: sensitive workflow changes such as approval actions, lock/unlock operations, role changes, access-tier changes, and compliance status updates must be validated server-side and tied to objects the caller is allowed to modify.

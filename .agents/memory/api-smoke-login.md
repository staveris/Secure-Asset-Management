...
**How to apply:** Don't guess passwords. Generate a bcrypt hash with `node -e "require('bcryptjs').hashSync('...',12)"`, UPDATE users.password_hash via psql, log in with curl `-c cookies`, and restore the documented seed password afterwards.
